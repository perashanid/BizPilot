...
import { getReceivablesAging, getPayablesAging } from '@/lib/financials';
...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  // Validated for consistency with the other analytics routes; the aging calculations
  // themselves are always "as of now" and don't take a date range.
  resolveRange(req.nextUrl.searchParams);

  const type = req.nextUrl.searchParams.get('type') === 'payable' ? 'payable' : 'receivable';
  const result = type === 'payable'
    ? await getPayablesAging(session.businessId)
    : await getReceivablesAging(session.businessId);
  return ok(result);
});
