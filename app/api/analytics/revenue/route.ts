...
import { getRevenueVsExpensesByPeriod } from '@/lib/financials';
...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  const granularity = req.nextUrl.searchParams.get('granularity') === 'day' ? 'day' : 'month';
  const result = await getRevenueVsExpensesByPeriod(session.businessId, { from, to }, granularity);
  return ok(result);
});
